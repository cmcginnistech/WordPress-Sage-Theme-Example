...
(function() {
  ...
  var CurtainMode = function(args) {
    ...
    this.tracker = new OpenSeadragon.MouseTracker({
      ...
      pressHandler: function(event) {
        ...
        var viewerPos = new OpenSeadragon.Point(
          self.viewer.container.clientWidth * self.clipFactorX,
        ...
        if (shownImages.length === 2) {
          ...
          if (
            Math.abs(viewerPos.x - event.position.x) < threshold &&
            event.position.y < viewerPos.y + threshold
          ) {
            self.isDraggingClipX = true;
          }
  ...
  CurtainMode.prototype = OpenSeadragon.extend(
    {
      ...
      updateClip: function() {
        var viewerPos = new OpenSeadragon.Point(
          this.viewer.container.clientWidth * this.clipFactorX,
    ...
    this.images.forEach(function(image, i) {
      ...
      image.sync.viewer.addHandler("open", function() {
        ...
        if (self.startingPan) {
          ...
          var pan = new OpenSeadragon.Point(
      ...
      var changeHandler = function() {
        ...
        self.images.forEach(function(image2) {
          ...
          image2.sync.viewer.viewport.zoomTo(
            image.sync.viewer.viewport.getZoom()
          );
          image2.sync.viewer.viewport.panTo(
            image.sync.viewer.viewport.getCenter()
          );
        });
  ...
  // ----------
  window.LeidenViewer = function(args) {
    var self = this;

    console.assert(args, "[LeidenViewer] args is required");
    console.assert(args.container, "[LeidenViewer] args.container is required");
    console.assert(args.images, "[LeidenViewer] args.images is required");
    console.assert(
      args.images.length > 0,
      "[LeidenViewer] args.images must have at least 1 image"
    );
    ...
    args.images.forEach(function(argsImage, i) {
      console.assert(
        argsImage.key,
        "[LeidenViewer] args.images[" + i + "].key is required"
      );
      console.assert(
        argsImage.tileSource,
        "[LeidenViewer] args.images[" + i + "].tileSource is required"
      );
  ...
  // ----------
  window.LeidenViewer.prototype = OpenSeadragon.extend(
    {
      // ----------
      getMode: function() {
        return this.modeKey;
      ...
      setMode: function(key) {
        ...
        console.assert(
          key === "curtain" || key === "sync",
          "[LeidenViewer.setMode] Must have valid key."
        );
        if (key === this.modeKey) {
          return;
        }
      ...
      setZoom: function(zoom) {
        console.assert(
          typeof zoom === "number" && zoom > 0 && zoom < Infinity,
          "[LeidenViewer.setZoom] zoom must be a positive number"
        );
      ...
      setPan: function(pan) {
        console.assert(
          typeof pan === "object" &&
            typeof pan.x === "number" &&
            typeof pan.y === "number",
          "[LeidenViewer.setPan] pan must be an object with an x and a y"
        );
